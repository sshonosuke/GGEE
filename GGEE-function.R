...
# GGEE: computing grouped GEE estimates given the number of groups
# GGEE.AV: asymptotic variance-covariance matrix of grouped GEE estimates
...
GGEE <- function(Y, X, G=2, maxit=1000, cor="EC", init="sep-reg", print=F){
  th <- 0.0001
  ep <- 10^(-5)
  n <- dim(Y)[1]
  TT <- dim(Y)[2]
  p <- dim(X)[3]
  N <- n*TT
  if(p>=TT & init=="sep-reg"){ 
    print("method sep-reg cannot be applied. mix-reg is used.") 
    init <- "mix-reg"
  }
  ...
  # Summary
  dimnames(hBeta)[[2]] <- paste0("group", 1:G)
  dimnames(hBeta)[[1]] <- paste0("coef", 1:p)
  Res <- list(Beta=hBeta, g=hg, Cor=hR, itr=k, init=init.fit)
  return(Res)
}
...
GGEE.AV <- function(Y, X, hBeta, hg, hR){
  th <- 0.0001
  G <- max(hg)
  n <- dim(Y)[1]
  p <- dim(X)[3]
...
##  Cluster Bootstrap for asymptotic variance-covariance matrix   ##
# Y: (n,TT)-matrix of binary response 
# (n: the number of subjects; TT: the number of repeated measurements)
# X: (n,TT,p)-array of covariates including an intercept
# (p: the number of covariates)
...
GGEE.AV.boot <- function(Y, X, G=2, B=50, cor="EC", pr=F){
  # preparation 
  n <- dim(Y)[1]
  p <- dim(X)[3]
  Beta.set <- array(NA, c(B, p, G))
  # initial estimate 
  fit <- GGEE(Y, X, G=G, cor=cor)
  hB <- fit$Beta
  # bootstrap replication 
  for(b in 1:B){
    id <- sample(1:n, n, replace=T)
    boot.fit <- GGEE(Y[id,], X[id,,], G=G, cor=cor)
    boot.Beta <- boot.fit$Beta
    Ind <- c()
    for(k in 1:G){
      Ind[k] <- which.min(apply((boot.Beta-hB[,k])^2, 2, sum))
    }
    Beta.set[b,,] <- boot.Beta[,Ind]
    if(round(b/10)==(b/10) & pr){ print(b) }
  }
...
# cor: working correlations (4 options; see 'GGEE' function )
...
CV <- function(Y, X, Gm=5, num=10, cor="EC", print=F, maxit=30, init="sep-reg"){
  ...
  for(k in 2:Gm){
    ...
    for(s in 1:num){
      ...
      # fitting
      fit1 <- GGEE(Y[Ind[,1],], X[Ind[,1],,], G=k, cor=cor, maxit=maxit, init=init)
      hR1 <- fit1$Cor
      hBeta1 <- fit1$Beta
      fit2 <- GGEE(Y[Ind[,2],], X[Ind[,2],,], G=k, cor=cor, maxit=maxit, init=init)
      hR2 <- fit2$Cor
      hBeta2 <- fit2$Beta
      # grouping 
      nn <- length(om)
